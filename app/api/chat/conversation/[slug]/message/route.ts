...
export const POST = withWidgetAuth<{ slug: string }>(async ({ request, context: { params } }, { session }) => {
  ...
  const { content, attachments = [], tools, customerSpecificTools } = createMessageBodySchema.parse(
    await request.json(),
  );
  ...
  if (tools) {
    await cacheTools(
      conversation.unused_mailboxId,
      tools,
      customerSpecificTools ? conversation.emailFrom : null,
    );
  }
